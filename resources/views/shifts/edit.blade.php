                    ...
                    <form action="{{ route('shifts.update', $shift) }}" method="POST">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="flex items-center mt-3">
                                <input id="next_day" name="next_day" type="checkbox" value="1" {{ old('next_day', $shift->next_day) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                <label for="next_day" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Next Day</label>
                            </div>

                            <div class="mt-3">
                                <label for="shift_group" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Shift Group</label>
                                <input type="text" id="shift_group" name="shift_group" class="{{ $errors->has('shift_group') ? 'bg-red-50 border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500' : 'bg-gray-50 border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500' }} border text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Enter shift group" value="{{ old('shift_group', $shift->shift_group) }}" required />
                                @error('shift_group')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="flex items-center mt-3">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $shift->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
